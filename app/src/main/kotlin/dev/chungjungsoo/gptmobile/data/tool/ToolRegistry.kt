                ...
                parameters = buildJsonObject {
                    ...
                    put("properties", buildJsonObject {
                        put("query", buildJsonObject {
                            put("type", "string")
                            put("description", "The exact web search query to run. Prefer a concise, specific query that names the topic, entity, date, or source when helpful.")
                        })
                        put("top_k", buildJsonObject {
                            put("type", "integer")
                            put("minimum", 1)
                            put("maximum", 10)
                            put("description", "Maximum number of results to return. Use a smaller value for focused searches and a larger value when the first page may be noisy.")
                        })
                    })
                ...
                parameters = buildJsonObject {
                    ...
                    put("properties", buildJsonObject {
                        put("url", buildJsonObject {
                            put("type", "string")
                            put("description", "A public HTTP or HTTPS URL that can be fetched without login, private network access, or special credentials.")
                        })
                    })
...
class DefaultToolRegistry @Inject constructor() : ToolRegistry {
    override fun toolsFor(platform: PlatformV2): List<ToolDefinition> = if (
        ...
        platform.compatibleType in setOf(ClientType.OPENAI, ClientType.OPENROUTER, ClientType.OLLAMA, ClientType.CUSTOM)
    ) {
